...
class TwoGuardsGame:
    ...
    def _reset_state(self) -> None:
        ...
        logger.debug(
            ...
            self.safe_door, self.truth_guard,
        )
    ...
    def _choose_door(self, door: int) -> None:
        ...
        logger.debug(
            ...
            door, self.safe_door, self.state,
        )
        self._draw()
    ...
    def _draw_intro(self) -> None:
        self.ax.set_title("The Two Guards", fontsize=18, fontweight="bold", color=ACCENT_COLOR)
        for i, line in enumerate(INTRO_LINES):
            self.ax.text(
                5, 7.2 - i * 0.55, line,
                fontsize=11, ha="center", color=ACCENT_COLOR if line.startswith('"') else TEXT_COLOR,
            )
        self.ax.text(
            5, 0.4, "Press SPACE to begin",
            fontsize=12, ha="center", color=ACCENT_COLOR,
            bbox=dict(facecolor=BG_COLOR, edgecolor=ACCENT_COLOR, boxstyle="round,pad=0.4"),
        )

    def _draw_ask(self) -> None:
        self.ax.set_title("Choose a guard to ask", fontsize=16, fontweight="bold", color=ACCENT_COLOR)
        self.ax.text(5, 6.5, "You may ask ONE guard:", fontsize=13, ha="center", color=TEXT_COLOR)
        self.ax.text(
            5, 5.8,
            '"Which door would the OTHER guard say leads to freedom?"',
            fontsize=11, ha="center", color=ACCENT_COLOR, style="italic",
        )
        self.ax.text(3, 3.5, "Guard 1", fontsize=16, ha="center", color=TEXT_COLOR, fontweight="bold")
        self.ax.text(7, 3.5, "Guard 2", fontsize=16, ha="center", color=TEXT_COLOR, fontweight="bold")
        self.ax.text(3, 2.8, "Press 1 to ask", fontsize=11, ha="center", color=ACCENT_COLOR)
        self.ax.text(7, 2.8, "Press 2 to ask", fontsize=11, ha="center", color=ACCENT_COLOR)

    def _draw_choose(self) -> None:
        self.ax.set_title("Now choose your door", fontsize=16, fontweight="bold", color=ACCENT_COLOR)
        ...
        for i, line in enumerate(question.split("\n")):
            self.ax.text(5, 7.0 - i * 0.6, line, fontsize=12, ha="center", color=TEXT_COLOR)

        self.ax.text(5, 5.5, answer, fontsize=13, ha="center", color=ACCENT_COLOR, fontweight="bold")
        self.ax.text(5, 4.6, HINT,   fontsize=11, ha="center", color=TEXT_COLOR,   style="italic")

        self.ax.text(3, 3.0, "Door 1", fontsize=16, ha="center", color=TEXT_COLOR, fontweight="bold")
        self.ax.text(7, 3.0, "Door 2", fontsize=16, ha="center", color=TEXT_COLOR, fontweight="bold")
    ...
    def _draw_end(self) -> None:
        ...
        self.ax.text(
            5, 2.0, "Press space or click to continue",
            fontsize=11, ha="center", color=ACCENT_COLOR,
        )
