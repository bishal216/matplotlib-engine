...
class PongGame:
    def __init__(self, fig, ax, width: int = 10, height: int = 8):
        ...
        self._key_cid  = self.fig.canvas.mpl_connect("key_press_event",   self._on_key_press)
        self._keyup_cid = self.fig.canvas.mpl_connect("key_release_event", self._on_key_release)
    ...
    def _apply_paddle_input(self) -> None:
        ...
        if "up" in self._keys_held:
            self.paddle_y = float(np.clip(
                self.paddle_y + PADDLE_SPEED, 0, self.height - PADDLE_HEIGHT
            ))
    ...
    def _update(self, frame) -> list:
        ...
        if ball_left <= paddle_right and ball_right >= PADDLE_X:
            ...
            if ball_bottom <= self.paddle_y + PADDLE_HEIGHT and ball_top >= self.paddle_y:
    ...
    def _draw(self) -> None:
        ...
        for y in np.arange(0.5, self.height, 1.0):
            self.ax.plot(
                [self.width / 2, self.width / 2], [y, y + 0.5],
                color=WALL_COLOR, linewidth=1,
            )

        # Paddle
        self.ax.add_patch(patches.Rectangle(
            (PADDLE_X, self.paddle_y), PADDLE_WIDTH, PADDLE_HEIGHT,
            facecolor=PADDLE_COLOR,
        ))

        # Ball
        self.ax.add_patch(patches.Circle(
            self.ball_pos, BALL_RADIUS, facecolor=BALL_COLOR,
        ))
