...
class PongGame:
    ...
    def setup_controls(self):
        keyboard.on_press_key('up', lambda _: self.move_paddle(-1))
        keyboard.on_press_key('down', lambda _: self.move_paddle(1))
    ...
    def draw(self):
        ...
        if self.game_over:
            ...
            self.ax.set_title(f'Pong - Score: {self.score}', fontsize=16, fontweight='bold')
        self.ax.text(0.5, 0.1, 'Up/Down: Move | R: Restart | ESC: Quit', transform=self.ax.transAxes, fontsize=10, color='white')
    
    def run(self):
        print('Pong Game Started!')
        print('Controls: Up/Down=Move, R=Restart, ESC=Quit')
...
def run_pong_game(fig, ax):
    try:
        game = PongGame(fig, ax)
        game.run()
